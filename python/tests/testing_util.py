...
def run_test_general(
    ...
    # 1. Base Magnet
    magnet = magnet_class(position=(0.1, 0.2, 0.3), orientation=rotation, **kwargs)
    ...
    small_magnet = magnet_class(
        position=(0.03, 0.02, 0.01), orientation=rotation, **small_kwargs
    )
    expected_small = load_test_array(f"{name}-small.csv")
    _check_close(
        _compute_field(small_magnet, points_small),
        expected_small,
        rtol,
        atol,
        max_mismatches,
    ...
    _check_close(
        _compute_field(magnet, points), expected_trans, rtol, atol, max_mismatches
    )
    ...
    _check_close(
        _compute_field(magnet, points), expected_rot, rtol, atol, max_mismatches
    )
    ...
    _check_close(
        _compute_field(magnet, points), expected_rot_trans, rtol, atol, max_mismatches
    )
