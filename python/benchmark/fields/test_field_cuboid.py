...
def magba_args():
    # position, orientation, dimensions, polarization
    return (0, 0, 0), Rotation.identity(), (0.1, 0.2, 0.3), (1, 2, 3)


@pytest.fixture(scope="function")
def magpy_args():
    # dimensions, polarization
    return np.array([[0.1, 0.2, 0.3]] * 10000), np.array([[1, 2, 3]] * 10000)
...
def compute_magba(observers, args):
    pymagba.fields.cuboid_B(observers, *args)


def compute_magpy(observers, args):
    magpylib._src.fields.field_BH_cuboid._BHJM_magnet_cuboid("B", observers, *args)


def test_cuboid_field_magba(benchmark, observers, magba_args):
    benchmark(compute_magba, observers, magba_args)


def test_cuboid_field_magpy(benchmark, observers, magpy_args):
