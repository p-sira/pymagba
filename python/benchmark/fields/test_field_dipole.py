...
def magba_args():
    # position, orientation, moment
    return (0, 0, 0), Rotation.identity(), (1, 2, 3)


@pytest.fixture(scope="function")
def magpy_args():
    # moment
    return (np.array([[1, 2, 3]] * 10000),)
...
def compute_magba(observers, args):
    pymagba.fields.dipole_B(observers, *args)


def compute_magpy(observers, args):
    magpylib._src.fields.field_BH_dipole._BHJM_dipole("B", observers, *args)


def test_dipole_field_magba(benchmark, observers, magba_args):
    benchmark(compute_magba, observers, magba_args)


def test_dipole_field_magpy(benchmark, observers, magpy_args):
